...
async function handleCallback(bot, chatId, data, query) {
  ...
  switch (data) {
    case `${P}_create`:
      bot.sendMessage(chatId, '📝 Nom d\'utilisateur VMESS:');
    ...
    default:
      if (data.startsWith(`${P}_del_`)) {
        ...
        bot.sendMessage(chatId, `⚠️ Supprimer *${user}* ?`, {
          ...
          reply_markup: { inline_keyboard: [
            [{ text: '🗑 Supprimer', callback_data: `${P}_dely_${user}` }, { text: '❌ Annuler', callback_data: `${P}_deln_${user}` }]
          ]}
        });
      }
        ...
        bot.sendMessage(chatId, `✏️ Que modifier pour *${user}* ?`, {
          parse_mode: 'Markdown',
          reply_markup: { inline_keyboard: [
            [{ text: '👤 Username', callback_data: `${P}_mu_${user}` }, { text: '🔑 UUID', callback_data: `${P}_mp_${user}` }],
            [{ text: '🔙 Retour', callback_data: `${P}_modify` }]
          ]}
        });
      }
      ...
      else if (data.startsWith(`${P}_mp_`)) await regenerateUUID(bot, chatId, data.replace(`${P}_mp_`, ''));
      else if (data.startsWith(`${P}_ren_`)) {
        const user = data.replace(`${P}_ren_`, '');
        bot.sendMessage(chatId, `🔄 *${user}* — Action:`, {
          parse_mode: 'Markdown',
          reply_markup: { inline_keyboard: [
            [{ text: '➕ Ajouter', callback_data: `${P}_ra_${user}` }, { text: '➖ Retirer', callback_data: `${P}_rs_${user}` }],
            [{ text: '🔙 Retour', callback_data: `${P}_renew` }]
          ]}
        });
      }
        ...
        bot.sendMessage(chatId, `⏱ Unité:`, {
          reply_markup: { inline_keyboard: [
            [{ text: '📅 Jours', callback_data: `${P}_ru_${add ? 'a' : 's'}_d_${user}` }],
            [{ text: '🕐 Heures', callback_data: `${P}_ru_${add ? 'a' : 's'}_h_${user}` }],
            [{ text: '⏱ Minutes', callback_data: `${P}_ru_${add ? 'a' : 's'}_m_${user}` }],
          ]}
        });
      }
...
async function handleCreateFlow(bot, chatId, text, pending, pendingActions) {
  if (pending.step === 'username') { pending.username = text.trim(); pending.step = 'days'; bot.sendMessage(chatId, '📅 Durée (jours):'); }
    ...
    if (text.trim() !== '0') {
      ...
      if (dataLimitBytes === null) { bot.sendMessage(chatId, '❌ Format invalide.'); return; }
    }
...
async function createUser(bot, chatId, username, days, connLimit, dataLimitBytes) {
  try {
    ...
    const vmessConfig = Buffer.from(JSON.stringify({ v: "2", ps: `${username}_WS-TLS`, add: domain, port: "443", id: uuid, aid: "0", scy: "auto", net: "ws", type: "none", host: domain, path: "/vmess", tls: "tls", sni: domain, alpn: "" })).toString('base64');
    const vmessNtls = Buffer.from(JSON.stringify({ v: "2", ps: `${username}_WS-NTLS`, add: domain, port: "80", id: uuid, aid: "0", scy: "auto", net: "ws", type: "none", host: domain, path: "/vmess", tls: "", sni: "", alpn: "" })).toString('base64');
    const grpcConfig = Buffer.from(JSON.stringify({ v: "2", ps: `${username}_gRPC`, add: domain, port: "443", id: uuid, aid: "0", scy: "auto", net: "grpc", type: "gun", host: "", path: "vmess-grpc", tls: "tls", sni: domain, alpn: "" })).toString('base64');
...
async function handleModifyUsername(bot, chatId, text, pending, pendingActions) {
  ...
  try {
    ...
    await runCommand(`rm -f ${USERS_DB}/${pending.user}.json`);
    await runCommand('systemctl restart xray');
...
async function handleRenewFlow(bot, chatId, text, pending, pendingActions) {
  ...
  try {
    ...
    bot.sendMessage(chatId, `✅ VMESS *${pending.user}* ${pending.sign === 'a' ? '+' : '-'}${amount} ${unitLabels[pending.unit]} → *${newExpiry}*`, { parse_mode: 'Markdown' });
  } catch (err) { bot.sendMessage(chatId, `❌ Erreur: ${err.message}`); }
}
...
async function listUsers(bot, chatId) {
  const users = await getUsers();
  if (!users.length) { bot.sendMessage(chatId, '📋 Aucun utilisateur VMESS.'); return; }
  let text = '━━━━━━━━━━━━━━━━━━━━━\n📋 *VMESS Users*\n━━━━━━━━━━━━━━━━━━━━━\n';
...
async function showTraffic(bot, chatId, username) {
  try {
    ...
    text += '\n━━━━━━━━━━━━━━━━━━━━━';
    bot.sendMessage(chatId, text, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔄 Actualiser', callback_data: `vmess_trr_${username}` }], [{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
  } catch (err) { bot.sendMessage(chatId, `❌ Erreur: ${err.message}`); }
}
