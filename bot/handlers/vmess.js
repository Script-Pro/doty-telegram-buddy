...
async function handleCallback(bot, chatId, data, query, pendingActions) {
  const msgId = query?.message?.message_id; const P = 'vmess';
...
async function regenerateUUID(bot, chatId, msgId, username, userId) {
  try { const info = JSON.parse(await runCommand(`cat ${USERS_DB}/${username}.json`)); const newUUID = generateUUID(); await updateClientField(XRAY_PROTO, username, 'id', newUUID); info.uuid = newUUID; await runCommand(`echo '${JSON.stringify(info)}' > ${USERS_DB}/${username}.json`); audit.log(userId, PROTO, `UUID régénéré: ${username}`); editOrSend(bot, chatId, msgId, `✅ UUID: \`${newUUID}\``, { parse_mode: 'Markdown', reply_markup: backBtns() });
...
async function toggleLock(bot, chatId, msgId, username, userId) {
  try { const info = JSON.parse(await runCommand(`cat ${USERS_DB}/${username}.json`)); const nl = !info.locked; info.locked = nl; await runCommand(`echo '${JSON.stringify(info)}' > ${USERS_DB}/${username}.json`);
    ...
    else await addClient(XRAY_PROTO, { id: info.uuid, alterId: 0, email: username, level: 0 });
    audit.log(userId, PROTO, `${nl ? '🔒' : '🔓'} ${username}`); editOrSend(bot, chatId, msgId, `✅ VMESS *${username}* ${nl ? '🔒' : '🔓'}`, { parse_mode: 'Markdown', reply_markup: backBtns() });
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur: ${err.message}`, { reply_markup: backBtns() }); }
}
