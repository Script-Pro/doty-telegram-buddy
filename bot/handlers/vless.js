...
// ==========================================
// FONCTIONS UTILITAIRES SÉCURISÉES POUR JSON
// ==========================================

// Lit un utilisateur et gère les erreurs si le JSON est corrompu
async function readUser(username) {
  try {
    const raw = await runCommand(`cat ${USERS_DB}/${username}.json`);
    return JSON.parse(raw);
  } catch (err) {
    throw new Error(`Fichier corrompu ou introuvable pour l'utilisateur ${username}.`);
  }
}

// Écrit un utilisateur de manière sécurisée (échappe les apostrophes pour Bash)
async function writeUser(username, data) {
  const jsonStr = JSON.stringify(data).replace(/'/g, "'\\''");
  await runCommand(`mkdir -p ${USERS_DB} && echo '${jsonStr}' > ${USERS_DB}/${username}.json`);
}
...
async function getUsers() {
  try { 
    const r = await runCommand(`ls ${USERS_DB}/ 2>/dev/null | sed 's/.json//'`); 
    return r ? r.split('\n').filter(Boolean) : []; 
  }
  catch { return []; }
}
...
// ==========================================
// GESTIONNAIRE DES CALLBACKS (BOUTONS)
// ==========================================
...
async function handleCallback(bot, chatId, data, query, pendingActions) {
  ...
  // Informer Telegram que le clic a été reçu pour stopper l'animation de chargement
  if (query && query.id) {
    bot.answerCallbackQuery(query.id).catch(() => {});
  }

  // Envelopper l'exécution pour éviter que le bot ne plante silencieusement
  try {
    ...
    console.error(`Erreur VLESS Callback: ${err.message}`);
...
async function handleCreateFlow(bot, chatId, text, pending, pendingActions, userMsgId) {
  if (pending.step === 'username') { 
    ...
    pending.step = 'days'; 
    autoDeleteSend(bot, chatId, '📅 Durée (jours):', {}, userMsgId); 
  }
  else if (pending.step === 'days') {
    const d = parseInt(text);
    if (isNaN(d) || d < 1) { 
      delete pendingActions[chatId]; 
      return editOrSend(bot, chatId, null, '❌ Nombre de jours invalide.', { reply_markup: { inline_keyboard: [[{ text: '🔄 Réessayer', callback_data: `${PROTO}_create` }, { text: '❌ Annuler', callback_data: `menu_${PROTO}` }], [{ text: '🏠 ACCUEIL', callback_data: 'back_main' }]] } }); 
    }
    pending.days = d; 
    pending.step = 'connlimit'; 
    autoDeleteSend(bot, chatId, '🔢 Limite connexions (0=illimité):', {}, userMsgId);
  }
  else if (pending.step === 'connlimit') {
    const l = parseInt(text);
    if (isNaN(l) || l < 0) { 
      delete pendingActions[chatId]; 
      return editOrSend(bot, chatId, null, '❌ Limite invalide.', { reply_markup: { inline_keyboard: [[{ text: '🔄 Réessayer', callback_data: `${PROTO}_create` }, { text: '❌ Annuler', callback_data: `menu_${PROTO}` }], [{ text: '🏠 ACCUEIL', callback_data: 'back_main' }]] } }); 
    }
    pending.connLimit = l; 
    pending.step = 'datalimit'; 
    autoDeleteSend(bot, chatId, '📦 Limite données (ex: `5GB`, `0`=illimité):', { parse_mode: 'Markdown' }, userMsgId);
  }
  else if (pending.step === 'datalimit') {
    delete pendingActions[chatId]; 
    let dl = 0;
    if (text.trim() !== '0') { 
      dl = parseLimitToBytes(text.trim()); 
      if (dl === null) return editOrSend(bot, chatId, null, '❌ Format invalide. Ex: 5GB, 500MB, 1TB', { reply_markup: { inline_keyboard: [[{ text: '🔄 Réessayer', callback_data: `${PROTO}_create` }, { text: '❌ Annuler', callback_data: `menu_${PROTO}` }], [{ text: '🏠 ACCUEIL', callback_data: 'back_main' }]] } }); 
    }
...
async function createUser(bot, chatId, username, days, connLimit, dataLimitBytes, createdById, createdByName) {
  try {
    ...
    const userInfo = { 
      ...
      createdBy: createdByName || String(createdById || 'unknown'), 
      createdById: createdById || null, 
      createdAt: new Date().toISOString() 
    };
    
    // Utilisation de la nouvelle fonction sécurisée !
    await writeUser(username, userInfo);
...
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛`;
    ...
    bot.sendMessage(chatId, `❌ Erreur lors de la création: ${err.message}`, { reply_markup: backBtns() });
  }
}
...
async function handleModifyUsername(bot, chatId, text, pending, pendingActions) {
  ...
  try {
    const info = await readUser(pending.user);
    await renameClient(XRAY_PROTO, pending.user, newUser);
    info.username = newUser;
    await writeUser(newUser, info);
    await runCommand(`rm -f ${USERS_DB}/${pending.user}.json`); // Supprimer l'ancien fichier
  ...
  } catch (err) { bot.sendMessage(chatId, `❌ Erreur de modification: ${err.message}`, { reply_markup: backBtns() }); }
}

async function regenerateUUID(bot, chatId, msgId, username, userId) {
  try {
    const info = await readUser(username);
    const newUUID = generateUUID();
    await updateClientField(XRAY_PROTO, username, 'id', newUUID);
    info.uuid = newUUID; 
    await writeUser(username, info);
...
async function handleRenewFlow(bot, chatId, text, pending, pendingActions) {
  ...
  try {
    ...
    await writeUser(pending.user, info);
  ...
  } catch (err) { bot.sendMessage(chatId, `❌ Erreur de renouvellement: ${err.message}`, { reply_markup: backBtns() }); }
}
...
async function deleteUser(bot, chatId, msgId, username, userId) {
  ...
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur suppression: ${err.message}`, { reply_markup: backBtns() }); }
}
...
async function listUsers(bot, chatId, msgId) {
  ...
  for (const u of users) { 
    try { 
      const d = await readUser(u); 
      text += `👤 ${u} | 📅 ${d.expiry} | ${d.locked ? '🔒' : '🔓'}\n`; 
    } catch { 
      text += `👤 ${u} (Erreur de fichier)\n`; 
    } 
  }
...
async function showDetail(bot, chatId, msgId, username) {
  ...
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur lecture détails: ${err.message}`, { reply_markup: backBtns() }); }
}

async function toggleLock(bot, chatId, msgId, username, userId) {
  try {
    const info = await readUser(username);
    const nl = !info.locked;
    info.locked = nl; 
    await writeUser(username, info);
  ...
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur de verrouillage: ${err.message}`, { reply_markup: backBtns() }); }
}
...
async function showTraffic(bot, chatId, msgId, username) {
  ...
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur de trafic: ${err.message}`, { reply_markup: backBtns() }); }
}
...
async function handleQuotaFlow(bot, chatId, text, pending, pendingActions) {
  ...
  if (text.trim() === '0') { 
    ...
    return bot.sendMessage(chatId, `✅ Quota supprimé`, { parse_mode: 'Markdown', reply_markup: backBtns() }); 
  }
...
async function handleConnLimitFlow(bot, chatId, text, pending, pendingActions) {
  ...
  try { 
    ...
    console.error("Erreur sauvegarde info connLimit", err);
  }
