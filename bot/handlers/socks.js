...
async function handleCallback(bot, chatId, data, query, pendingActions) {
  ...
  switch (data) {
    ...
    default:
      ...
      else if (data.startsWith(`${P}_mp_`)) { const u = data.replace(`${P}_mp_`, ''); editOrSend(bot, chatId, msgId, `🔑 Nouveau mot de passe pour *${u}*:`, { parse_mode: 'Markdown' }); pendingActions[chatId] = { action: `${P}_modify_pass`, user: u, handler: handleModifyPassword }; }
...
async function handleCreateFlow(bot, chatId, text, pending, pendingActions) {
  if (pending.step === 'username') { pending.username = text.trim(); pending.step = 'password'; bot.sendMessage(chatId, '🔑 Mot de passe:'); }
    ...
    await createUser(bot, chatId, pending.username, pending.password, pending.days, pending.connLimit, dl);
  }
}
...
async function handleRenewFlow(bot, chatId, text, pending, pendingActions) {
  ...
  try {
    ...
    bot.sendMessage(chatId, `✅ SOCKS *${pending.user}* ${pending.sign==='a'?'+':'-'}${amount} ${{d:'jour(s)',h:'heure(s)',m:'minute(s)'}[pending.unit]} → *${newExpiry}*`, { parse_mode: 'Markdown' });
  } catch (err) { bot.sendMessage(chatId, `❌ Erreur: ${err.message}`); }
}
...
async function deleteUser(bot, chatId, msgId, username) {
  try {
    ...
    await runCommand('systemctl restart xray');
    editOrSend(bot, chatId, msgId, `✅ SOCKS *${username}* supprimé.`, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur: ${err.message}`); }
}

async function listUsers(bot, chatId, msgId) {
  const users = await getUsers();
  if (!users.length) { editOrSend(bot, chatId, msgId, '📋 Aucun utilisateur SOCKS.', { reply_markup: { inline_keyboard: [[{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } }); return; }
  let text = '━━━━━━━━━━━━━━━━━━━━━\n📋 *SOCKS Users*\n━━━━━━━━━━━━━━━━━━━━━\n';
...
async function toggleLock(bot, chatId, msgId, username) {
  try {
    ...
    if (nl) await runCommand(`cd /etc/xray && jq 'del(.inbounds[${INBOUND_INDEX}].settings.accounts[] | select(.user=="${username}"))' config.json > tmp.json && mv tmp.json config.json`);
    ...
    await runCommand('systemctl restart xray');
    editOrSend(bot, chatId, msgId, `✅ SOCKS *${username}* ${nl?'🔒':'🔓'}`, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur: ${err.message}`); }
}
...
async function showTraffic(bot, chatId, msgId, username) {
  try {
    ...
    text += '\n━━━━━━━━━━━━━━━━━━━━━';
    editOrSend(bot, chatId, msgId, text, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔄 Actualiser', callback_data: `socks_trr_${username}` }], [{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur: ${err.message}`); }
}
