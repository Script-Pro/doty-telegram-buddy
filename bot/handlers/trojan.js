...
async function handleCallback(bot, chatId, data, query, pendingActions) {
  ...
  switch (data) {
    ...
    default:
      ...
      else if (data.startsWith(`${P}_mod_`)) { const u = data.replace(`${P}_mod_`, ''); editOrSend(bot, chatId, msgId, `✏️ Modifier *${u}*:`, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '👤 Username', callback_data: `${P}_mu_${u}` }, { text: '🔑 Password', callback_data: `${P}_mp_${u}` }], [{ text: '🔙 Retour', callback_data: `${P}_modify` }]] } }); }
      ...
      else if (data.startsWith(`${P}_mp_`)) await regeneratePassword(bot, chatId, msgId, data.replace(`${P}_mp_`, ''));
...
async function regeneratePassword(bot, chatId, msgId, username) {
  try {
    const info = JSON.parse(await runCommand(`cat ${USERS_DB}/${username}.json`)); const newPass = generateUUID().split('-')[0];
    ...
    info.password = newPass; await runCommand(`echo '${JSON.stringify(info)}' > ${USERS_DB}/${username}.json`);
    await runCommand('systemctl restart xray');
    editOrSend(bot, chatId, msgId, `✅ Nouveau password pour *${username}*:\n\`${newPass}\``, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
...
async function handleRenewFlow(bot, chatId, text, pending, pendingActions) {
  ...
  try {
    ...
    bot.sendMessage(chatId, `✅ TROJAN *${pending.user}* ${pending.sign==='a'?'+':'-'}${amount} ${{d:'jour(s)',h:'heure(s)',m:'minute(s)'}[pending.unit]} → *${newExpiry}*`, { parse_mode: 'Markdown' });
  } catch (err) { bot.sendMessage(chatId, `❌ Erreur: ${err.message}`); }
}
...
async function deleteUser(bot, chatId, msgId, username) {
  try {
    ...
    await runCommand('systemctl restart xray');
    editOrSend(bot, chatId, msgId, `✅ TROJAN *${username}* supprimé.`, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur: ${err.message}`); }
}

async function listUsers(bot, chatId, msgId) {
  const users = await getUsers();
  if (!users.length) { editOrSend(bot, chatId, msgId, '📋 Aucun utilisateur TROJAN.', { reply_markup: { inline_keyboard: [[{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } }); return; }
  let text = '━━━━━━━━━━━━━━━━━━━━━\n📋 *TROJAN Users*\n━━━━━━━━━━━━━━━━━━━━━\n';
...
async function toggleLock(bot, chatId, msgId, username) {
  try {
    ...
    await runCommand('systemctl restart xray');
    editOrSend(bot, chatId, msgId, `✅ TROJAN *${username}* ${nl?'🔒':'🔓'}`, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur: ${err.message}`); }
}
...
async function showTraffic(bot, chatId, msgId, username) {
  try {
    ...
    text += '\n━━━━━━━━━━━━━━━━━━━━━';
    editOrSend(bot, chatId, msgId, text, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔄 Actualiser', callback_data: `trojan_trr_${username}` }], [{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
  } catch (err) { editOrSend(bot, chatId, msgId, `❌ Erreur: ${err.message}`); }
}
