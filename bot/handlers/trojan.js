...
async function handleCallback(bot, chatId, data, query, pendingActions) {
  ...
  switch (data) {
    ...
    default:
      ...
      else if (data.startsWith(`${P}_mu_`)) { const u=data.replace(`${P}_mu_`,''); editOrSend(bot,chatId,msgId,`📝 Nouveau nom:`,{parse_mode:'Markdown'}); pendingActions[chatId]={action:`${P}_modify_user`,user:u,handler:handleModifyUsername,fromId:query.from.id}; }
...
async function createUser(bot,chatId,username,days,connLimit,dataLimitBytes,createdById,createdByName) {
  try {
    ...
    await runCommand(`cd /etc/xray && jq '.inbounds[${INBOUND_INDEX}].settings.clients += [{"password":"${password}","email":"${username}","level":0}]' config.json > tmp.json && mv tmp.json config.json`);
...
async function handleModifyUsername(bot,chatId,text,pending,pendingActions) {
  ...
  try { const info=JSON.parse(await runCommand(`cat ${USERS_DB}/${pending.user}.json`)); const n=text.trim(); await runCommand(`cd /etc/xray && jq '(.inbounds[${INBOUND_INDEX}].settings.clients[] | select(.email=="${pending.user}")).email = "${n}"' config.json > tmp.json && mv tmp.json config.json`); info.username=n; await runCommand(`echo '${JSON.stringify(info)}' > ${USERS_DB}/${n}.json && rm -f ${USERS_DB}/${pending.user}.json`); await runCommand('systemctl restart xray 2>/dev/null || true'); audit.log(pending.fromId,PROTO,`Modifié: ${pending.user} → ${n}`); bot.sendMessage(chatId,`✅ *${pending.user}* → *${n}*`,{parse_mode:'Markdown',reply_markup:backBtns()});
...
async function regeneratePassword(bot,chatId,msgId,username,userId) {
  try { const info=JSON.parse(await runCommand(`cat ${USERS_DB}/${username}.json`)); const np=generateUUID().split('-')[0]; await runCommand(`cd /etc/xray && jq '(.inbounds[${INBOUND_INDEX}].settings.clients[] | select(.email=="${username}")).password = "${np}"' config.json > tmp.json && mv tmp.json config.json`); info.password=np; await runCommand(`echo '${JSON.stringify(info)}' > ${USERS_DB}/${username}.json`); await runCommand('systemctl restart xray 2>/dev/null || true'); audit.log(userId,PROTO,`Password régénéré: ${username}`); editOrSend(bot,chatId,msgId,`✅ Nouveau password: \`${np}\``,{parse_mode:'Markdown',reply_markup:backBtns()});
...
async function deleteUser(bot,chatId,msgId,username,userId) {
  try { await runCommand(`cd /etc/xray && jq 'del(.inbounds[${INBOUND_INDEX}].settings.clients[] | select(.email=="${username}"))' config.json > tmp.json && mv tmp.json config.json`); await runCommand(`rm -f ${USERS_DB}/${username}.json`); await removeDataLimit(PROTO,username); await runCommand('systemctl restart xray 2>/dev/null || true'); audit.log(userId,PROTO,`Supprimé ${username}`); editOrSend(bot,chatId,msgId,`✅ TROJAN *${username}* supprimé.`,{parse_mode:'Markdown',reply_markup:backBtns()});
  } catch(err) { editOrSend(bot,chatId,msgId,`❌ Erreur: ${err.message}`,{reply_markup:backBtns()}); }
}
...
async function showDetail(bot,chatId,msgId,username) {
  try { const info=JSON.parse(await runCommand(`cat ${USERS_DB}/${username}.json`)); const domain=await getDomain(); const traffic=await getXrayTraffic(username); const limit=await getDataLimit(PROTO,username); const conn=await getConnLimit(PROTO,username); const online=await countXrayConnections(username);
...
async function toggleLock(bot,chatId,msgId,username,userId) {
  try { const info=JSON.parse(await runCommand(`cat ${USERS_DB}/${username}.json`)); const nl=!info.locked; info.locked=nl; await runCommand(`echo '${JSON.stringify(info)}' > ${USERS_DB}/${username}.json`);
    ...
    else await runCommand(`cd /etc/xray && jq '.inbounds[${INBOUND_INDEX}].settings.clients += [{"password":"${info.password}","email":"${username}","level":0}]' config.json > tmp.json && mv tmp.json config.json`);
    await runCommand('systemctl restart xray 2>/dev/null || true'); audit.log(userId,PROTO,`${nl?'🔒':'🔓'} ${username}`);
    editOrSend(bot,chatId,msgId,`✅ TROJAN *${username}* ${nl?'🔒':'🔓'}`,{parse_mode:'Markdown',reply_markup:backBtns()});
  } catch(err) { editOrSend(bot,chatId,msgId,`❌ Erreur: ${err.message}`,{reply_markup:backBtns()}); }
}
...
async function showOnline(bot,chatId,msgId,username) {
  try { const online=await countXrayConnections(username); const conn=await getConnLimit(PROTO,username);
