...
async function handleCallback(bot, chatId, data, query) {
  ...
  switch (data) {
    ...
    case `${P}_lockuser`: await showPaginatedList(bot, chatId, `${P}_lck_`, `${P}_pglk_`, 0); break;
...
async function listUsers(bot, chatId) {
  const users = await getUsers();
  if (!users.length) { bot.sendMessage(chatId, '📋 Aucun utilisateur SSH.'); return; }
  let text = '━━━━━━━━━━━━━━━━━━━━━\n📋 *SSH Users*\n━━━━━━━━━━━━━━━━━━━━━\n';
...
async function showTraffic(bot, chatId, username) {
  try {
    ...
    text += '\n━━━━━━━━━━━━━━━━━━━━━';
    bot.sendMessage(chatId, text, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🔄 Actualiser', callback_data: `ssh_trr_${username}` }], [{ text: '🔙 Retour', callback_data: `menu_${PROTO}` }]] } });
  } catch (err) { bot.sendMessage(chatId, `❌ Erreur: ${err.message}`); }
}
...
async function handleConnLimitFlow(bot, chatId, text, pending, pendingActions) {
  ...
  // Update system limits.conf
  try {
    await runCommand(`sed -i '/${pending.user}/d' /etc/security/limits.conf 2>/dev/null || true`);
    if (limit > 0) await runCommand(`echo "${pending.user} hard maxlogins ${limit}" >> /etc/security/limits.conf`);
